...
モードOFF中にモック内のリンクやJSで画面が変わった場合は、ライブ表示のコメント追加が止まり、ペインに「モック内で画面遷移したためコメントを追加できません。」というバナーが出る。表示中の画面と保存先ファイルがずれたまま貼ってしまうのを防ぐため。バナーの「このペインを再読込」で元のファイルに戻ると、再びコメントを貼れる。同じページ内のアンカーリンク ( `#section` ) は画面が変わらないので対象外。

### コメント本文のリンク

コメント本文に書いた `http://` / `https://` で始まるURLはリンクになる。要件定義mdの行リンクをコピーしてモック側のコメントに貼っておけば、後から読む人がその要件へ直接飛べる。

mdmiel自身の共有URL ( 行リンク・付箋リンク ) は同じタブで開き、それ以外は別タブで開く。共有URLかどうかはホストとハッシュで判定するため、`--port` を変えて起動し直しても以前貼ったリンクは同じタブで開く。
