...
func main() {
	...
	port := serveCmd.String("port", "8686", "Port to bind HTTP server")

	// flag は最初の非フラグ引数でパースを止めるため、
	// "serve <dir> --port N" と "serve --port N <dir>" の両形式に対応できるよう
	...
	if err := serveCmd.Parse(flagArgs); err != nil {
...
func printUsage() {
	fmt.Println("Usage:")
	fmt.Println("  mdmiel serve <dir> [--port 8686]")
}
